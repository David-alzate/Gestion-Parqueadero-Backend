...
public class EmpleadoPostgresqlDAO extends SqlConnection implements EmpleadoDAO {
	...
	public void crear(EmpleadoEntity data) {
		...
		sentenciaSql
				.append("sede_id, tipoidentificacion_id, tipoempleado_id, password, numeroidentificacion, estado_id)");
	...
	public List<EmpleadoEntity> consultar(EmpleadoEntity data) {
		...
		sentenciaSql.append("te.id as idTipoEmpleado, te.nombre as nombreTipoEmpleado, est.id as idEstado, est.estado as nombreEstado ");
		...
		sentenciaSql.append("INNER JOIN estado est ON e.estado_id = est.id ");
		...
		if (!ObjectHelper.getObjectHelper().isNull(data.getEstado())
			...
			sentenciaSql.append(" AND est.id = ?");
		...
		try (final PreparedStatement sentenciaSqlPreparada = getConexion().prepareStatement(sentenciaSql.toString())) {
			...
			try (final ResultSet resultado = sentenciaSqlPreparada.executeQuery()) {
				while (resultado.next()) {
					...
					estado.setEstado(resultado.getString("nombreEstado"));
	...
	public void modificar(EmpleadoEntity data) {
		...
		sentenciaSql.append(
				"UPDATE empleado SET nombre=?, apellido=?, correoelectronico=?, sede_id=?, tipoidentificacion_id=?,  ");
